...
public class MakeTable extends Super implements ActionListener
{
	/*
*********************************************************************
...
*/
	...
	// Number of grid points
        ...
        gridp.setBackground(scrn);
        gridp.setForeground(scrf);
	...
	// R coordinate 
        ...
        xval.setBackground(scrn);
        xval.setForeground(scrf);
        fix(xval,grd,gbc,0,6,1,1);

	// Function value

        yval = new JTextField(8);
        yval.setBackground(scrn);
        yval.setForeground(scrf);
	...
	// Function value
        ...
        points.setBackground(back);
        points.setForeground(fore);
	...
	// Energy units required
        ...
        units = new JComboBox();
        units.setBackground(scrn);
        units.setForeground(scrf);
	...
	// Energy units required
        ...
        fitopt.setBackground(scrn);
        fitopt.setForeground(scrf);
	...
	// Special potential functions
        ...
        special.setBackground(scrn);
        special.setForeground(scrf);
...
*/
	home=here;
	monitor.println("Activated panel for making TABLE files");
	job=new MakeTable();
	job.pack();
	job.show();
	header=new String[4];
...
*/
	...
	if (arg.equals("Make"))
	    ...
	    {
		if((tname=selectFileNameEnds(job,"XY"))!=null)
		    {
			npnts=getXY(tname,0);
			rcut=BML.giveDouble(cutoff.getText(),1);
			name1=BML.fmt(atom1.getText(),8);
...
*/
	...
	monitor.println("Making TABLE file .......");
	...
	if(keytab==0)
		...
		if(npnts > 0)
		    ...
		    {
			monitor.println("Error - no points to fit!");
		    }
	    }
	else if(keytab==1)
	    {
...
*/
	...
	monitor.println("Number of points loaded:"+BML.fmt(n,6));
	return n;
    }
...
*/
	...
	if(ngrid<6)
	    {         
		monitor.println("Error - incorrect grid number specified for TABLE file");
		return -1;
	    }
	else if(rcut<=0.0)
	    {
		monitor.println("Error - incorrect cutoff specified for TABLE file");
		return -2;
	    }

	// print current list of points and convert to dl_poly units

	monitor.println("Current list of spline points:");

	for(int i=0;i<npnts;i++)
	    {
		monitor.println(BML.fmt(i+1,6)+BML.fmt(xx[i],8)+BML.fmt(yy[i],8));
		yy[i]=ecnvrt*yy[i];
	    }

	// calculate spline terms

	spline(npnts,xx,yy,zz,aa,dd,gg);
...
*/
	...
	call=gaussfit(npnts,ccc,eee,xx,yy,zz,aa,dd,gg);
	if(call<0)return -3;

	// print out potential arrays for comparison
      
	monitor.println("       rad        y(i)        z(i)");
      
	for(int i=0;i<npnts;i++)
	    {
		x=xx[i];
		zz[i]=ccc[0]*Math.exp(-eee[0]*x*x)+ccc[1]*Math.exp(-eee[1]*x*x)+ccc[2]*Math.exp(-eee[2]*x*x);
		monitor.println(BML.fmt(x,12)+BML.fmt(yy[i],12)+BML.fmt(zz[i],12));
	    }
