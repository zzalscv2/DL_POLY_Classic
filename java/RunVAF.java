...
public class RunVAF extends Super implements ActionListener
{
	/*
*********************************************************************
...
*/
    ...
    static double[][] xyz,vel,frc,vaf;
    static double[][][] vaf0;
    static int npnts,nconf,nvaf,isampl,iovaf;
...
*/
	home=here;
	monitor.println("Activated VAF panel");
	job=new RunVAF();
	job.pack();
	job.show();
	npnts=0;
...
*/
	...
	if (arg.equals("Run"))
		...
		monitor.println("Started VAF calculation .....");
		npnts=calcVAF();
		if(npnts>0)
		    {
			vafXY(npnts,atname);
...
*/      
	...
	if(nvaf%iovaf != 0)
		...
		monitor.println("Warning - vaf array dimension reset to "+BML.fmt(nvaf,8));
	    }
	novaf=nvaf/iovaf;
	imd=new int[nvaf];
	msm=new int[nvaf];
	...
	monitor.println("Length of correlation arrays  : "+BML.fmt(nvaf,8));
	monitor.println("Number of configurations      : "+BML.fmt(nconf,8));
	monitor.println("Sampling interval             : "+BML.fmt(isampl,8));
	monitor.println("Interval between origins      : "+BML.fmt(iovaf,8));
