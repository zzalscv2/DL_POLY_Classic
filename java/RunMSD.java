...
public class RunMSD extends Super implements ActionListener
{
	/*
*********************************************************************
...
*/
    ...
    static String[] name;
    static int[] imd,msm;
    static double[] chge,weight;
    static double[][] xyz,vel,frc,xy0,xy1,acm,msd;
    static double[][][] msd0;
    static int npnts,nconf,nmsd,isampl,iomsd;
    static JTextField atom1,history,configs,length,sample,origin;
...
*/
	home=here;
	monitor.println("Activated MSD panel");
	job=new RunMSD();
	job.pack();
	job.show();
	npnts=0;
...
*/
	...
	if (arg.equals("Run"))
		...
		monitor.println("Started MSD calculation .....");
		npnts=calcMSD();
		if(npnts>0)
		    {
			msdXY(npnts,atname);
...
*/      
	...
	double dt0,det,f1,f2,uuu,vvv,www,rmsx,rmsy,rmsz,rr2,tstep;
	...
	if(nmsd%iomsd != 0)
		...
		monitor.println("Warning - msd array dimension reset to "+BML.fmt(nmsd,8));
	    }
	nomsd=nmsd/iomsd;
	xx=new double[nmsd];
	yy=new double[nmsd];
	...
	monitor.println("Length of correlation arrays  : "+BML.fmt(nmsd,8));
	monitor.println("Number of configurations      : "+BML.fmt(nconf,8));
	monitor.println("Sampling interval             : "+BML.fmt(isampl,8));
	monitor.println("Interval between origins      : "+BML.fmt(iomsd,8));
	...
	if(form)
		...
		if(BML.nint(info[3])<0 && BML.nint(info[3])!=-1)
		    {
			monitor.println("Error - cannot open HISTORY file");
			return -1;
		    }
	    }
	else
