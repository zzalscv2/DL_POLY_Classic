...
public class MakeDreiField extends Super implements ActionListener
{
    /*
*********************************************************************
...
*/
    static GUI home;
    static MakeDreiField job;
    static JComboBox bond,vdwt;
    static JButton make,load,close;
    static JCheckBox display,charges;
    static JLabel lab1,lab2,pad1,pad2,pad3;
    static boolean lbuck,lmorse,lchg,lshow,lgetf,lfixed;
    static String[] dsym,kbnd,kang,kdih,kinv,kvdw,khbd;
    static String[][] idvdw,idhbd;
    static double[] dmss,dchg,drad,dang,deps,dzet,dsig;
    static double[] bord,eig;
    static double[][] pang,pbnd,pdih,pinv,phbd,pvdw,con,vec;
    static int mxpbnd,mxpang,mxpdih,mxpinv,mxphbd,mxpvdw,mxvdw,lhbnd;
    static int[] ident,jdent,ida,lab;
    static int nvdw,nhbd,ndrei;
...
*/
	...
	//        VDW type specification
        ...
        vdwt = new JComboBox();
        vdwt.setBackground(scrn);
        vdwt.setForeground(scrf);
	...
	charges = new JCheckBox("Use Charges");
        charges.setBackground(back);
        charges.setForeground(fore);
	...
	fix(display,grd,gbc,0,6,1,1);

	//        Pad out
	
	pad3 = new JLabel(" ");
	fix(pad3,grd,gbc,0,7,1,1);
...
*/
	...
	monitor.println("Activated panel for making Dreiding FIELD files");
      	home=here;
	lfixed=false;
	lbuck=false;
	lmorse=false;
...
*/
	...
	if(dreiding() != 0)
	    {
		monitor.println("Error - failed in processing MINIDREI file");
		return -1;
	    }
	...
	try
		...
		for(int m=0;m<struct.nmoltp;m++)
			...
			if(border(mol)!= 0)
			    {
				monitor.println("Warning - conjugation analysis failed");
				monitor.println("Using default bond orders (1.0)");
			    }
...
*/      
	...
	try
		...
		monitor.println("Reading file: MINIDREI");
		for(int i=0;i<6;i++)
		...
		ndrei=i;
		monitor.println("MINIDREI File successfully loaded");
		reader.close();
	    }
	catch(FileNotFoundException e)
	    {
		monitor.println("Error - file not found: MINIDREI");
		return -1;
	    }
	catch(Exception e)
	    {
		monitor.println("Error reading file: MINIDREI" + " "+e);
		return -2;
	    }
	return 0;
    }
...
copyright daresbury laboratory october 2000
...
// set default bond orders
	...
	// identify SP2 atoms
        ...
        monitor.println("Number of SP2 atoms found:" + BML.fmt(jbnd,6));
        monitor.println("Number PI electrons found:" + BML.fmt(nmpi,6));

	// initialize huckel matrix

        monitor.println("Starting conjugated bond analysis.....");
	...
	// diagonalise huckel matrix
        ...
        status=Jacobi(jbnd,con,vec);
        if(status != 0)
	    { 
        	monitor.println("Error - problem diagonalising bond order matrix");
		return -1;
	    }
	...
	// sort eigenvalues
        ...
        ShellSort(jbnd,lab,eig);
                
	// construct bond orders

        if(jbnd%2 > 0)
            monitor.println("Warning - possible uneven conjugation found");
        for (int k=0;k<mol.nbnd;k++)
	    {
		ii=ida[mol.ibnd[0][k]];
		jj=ida[mol.ibnd[1][k]];
	...
	// rationalise bond orders
        ...
        monitor.println("Finished conjugated bond analysis");
	return 0;
    }
...
*********************************************************************
    */
	...
	pbnd=new double[mxpbnd][mol.nbnd];
        ...
        monitor.println("Bond potentials assigned");
    }
...
*/
	...
	for(int m=0;m<mol.ndih;m++)
		...
		if(namj.charAt(2)=='3' && namk.charAt(2)=='3')
			...
			if(lnull)
			    {
				monitor.println("Warning - null dihedral parameters specified");
				lnull=false;
			    }
			kdih[m]="null";
			pdih[0][m]=0.0;
...
*/
	...
	if(newinv > 0) monitor.println("Inversion angles assigned");
	return newinv;
    }
...
*********************************************************************
	...
	if((namj.indexOf("C_")==0) && (namk.indexOf("C_")==0))
	    ...
	    {
		monitor.println("Warning - unknown potential in sp3-sp3 dihedral");
		kdih[m]="unkn";
		pdih[0][m]=0.0;
		pdih[1][m]=0.0;
		pdih[2][m]=0.0;
...
*/
	...
	if(namk.charAt(2)=='3')
		...
		if((namj.indexOf("C_")==0) && (namk.indexOf("C_")==0))
		    ...
		    {
			monitor.println("Warning - unknown potential in sp2-sp3 dihedral");
			kdih[m]="unkn";
			pdih[0][m]=0.0;
			pdih[1][m]=0.0;
			pdih[2][m]=0.0;
...
*/
	...
	if(mhbd > 0) monitor.println("H-bond terms assigned");
	return mhbd;
    }
}
