...
public class StatProp extends Super implements ActionListener
{
	/*
*********************************************************************
...
*/
    ...
    static StatProp job;
    static String prop;
    static JComboBox property;
    static int npnts,keyprp;
    static JTextField statis;
    static JButton run,close;
    static int[] key={0,1,2,3,4,18,26,25};
    static double[] xx,yy;
...
*/
	...
	// Name of STATIS file
        ...
        statis.setBackground(scrn);
        statis.setForeground(scrf);
	...
	fix(lab2,grd,gbc,0,3,1,1);
        ...
        property.setBackground(scrn);
        property.setForeground(scrf);
...
*/
	home=here;
	monitor.println("Activated statistics panel");
	job=new StatProp();
	job.pack();
	job.show();
	npnts=0;
...
*/
	...
	if (arg.equals("Run"))
		...
		monitor.println("Started statistics calculation .....");
		npnts=calcStats();
		if(npnts>0)
		    {
			statsXY(npnts,prop);
...
*/
	...
	// open the statistical data file
	    ...
	    {
		monitor.println("Error - STATIS file not found");
		return -2;
	    }
	catch(IOException e)
	    {
		monitor.println("Error - STATIS file processing error");
		try{lnr.close();}catch(IOException ee){}
		return -3;
	    }
	npnts=i;
	monitor.println("Number of data points processed: "+BML.fmt(npnts,8));
	...
	monitor.println("Calculated average: "+BML.fmt(aver,15));

	// calculate error using blocking method

	monitor.println("Standard error and error uncertainty");
	...
	monitor.println(BML.fmt(sigma[0],15)+"+/-"+BML.fmt(enigma[0],15));
	...
	while(mmm>3 && n<ndeg)
		...
		monitor.println(BML.fmt(sigma[n],15)+"+/-"+BML.fmt(enigma[n],15));
	    }
