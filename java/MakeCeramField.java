...
public class MakeCeramField extends Super implements ActionListener
{
	/*
*********************************************************************
...
*/
    static GUI home;
    static MakeCeramField job;
    static String ffield;
    static JComboBox cpot;
    static JButton make,load,close;
    static JCheckBox display,tetra;
    static JLabel lab1,pad;
    static boolean ltetr,lshow,lgetf;
    static String[] unqatm,name;
    static double[] mass,aaa,rho,ccc,cga,cgb,spa,spb,rad;
    static int natms,imcon,nunq,nmols,nrept,nshl,ntatm;
...
*/
	...
	//        Bond type specification
        ...
        cpot = new JComboBox();
        cpot.setBackground(scrn);
        cpot.setForeground(scrf);
	cpot.addItem("LC_a");
	cpot.addItem("LC_b");
	cpot.addItem("LC_c1");
	cpot.addItem("LC_c2");
	...
	tetra = new JCheckBox("Tetragonal ion");
        tetra.setBackground(back);
        tetra.setForeground(fore);
...
*/
	...
	monitor.println("Activated panel for making ceramic FIELD files");
      	home=here;
	ltetr=false;
        lshow=true;
	if(config==null)
...
*/
	...
	if(ceramics() != 0)
	    {
		monitor.println("Error - failed in processing CERAMICS file");
		return -1;
	    }
					...
					for(int j=i+1;j<nrept;j++)
						...
						if(name[i].substring(0,4).equals(name[j].substring(0,4)))
							...
							if(imcon > 0)
								...
								xd=(config.cell[0]*xss+config.cell[3]*yss+config.cell[6]*zss);
								yd=(config.cell[1]*xss+config.cell[4]*yss+config.cell[7]*zss);
								zd=(config.cell[2]*xss+config.cell[5]*yss+config.cell[8]*zss);
							    }
...
*/      
	...
	try
		...
		monitor.println("Reading file: CERAMICS");
	    OUT2:
		...
		last=i;
		monitor.println("CERAMICS File successfully loaded");
		reader.close();
	    }
	catch(FileNotFoundException e)
	    {
		monitor.println("Error - file not found: CERAMICS");
		return -1;
	    }
	catch(Exception e)
	    {
		monitor.println("Error reading file: CERAMICS" + " "+e);
		return -2;
	    }
	if(nas != nunq)
	    {
		monitor.println("Error - required atom type not in CERAMICS file");
		return -3;
	    }
	...
	if(ffield.equals("LC_a") || ffield.equals("LC_b"))
		...
		for(i=0;i<nunq;i++)
			...
			if(!unqatm[i].substring(4).equals("    "))
			    {
				monitor.println("Error - unrecognised forcefield atom: "+unqatm[i]);
				return -4;
			    }
		    }
	    }
		...
		if(ffield.equals("LC_c1") || ffield.equals("LC_c2"))
			...
			if(ntatm > 2)
			    {
				monitor.println("Force field not suitable for mixed oxides");
				return -6;
			    }
			for(i=0;i<nunq;i++)
			    {
