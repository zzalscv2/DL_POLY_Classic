...
public class GraphDraw extends Super implements ActionListener
{
    /*
*********************************************************************
...
*/
    static GUI home;
    static GUIPlot plot;
    static GraphDraw job;
    static String plab,xlab,ylab;
    static int keyplot,ngrid,line,npnts;
    static JButton load,spline,close,dots,lines,prin,zoom,clear;
    static JTextField xlabel,ylabel,plabel;
    static double[] uu,vv,xx,yy;
    static boolean ldots,lzoom,lmove,lscale;
    static String[] header;
    static boolean[] lshow;
    static int ptilex=600,ptiley=525; 
...
*/
	...
	plot.setBackground(scrn);
	plot.setForeground(scrf);
	...
	// Spline button
        ...
        spline.setBackground(butn);
        spline.setForeground(butf);
        fix(spline,grd,gbc,4,1,1,1,20,10);
	
	// Blank button 4
	
	dots = new JButton("Dots");
        dots.setBackground(butn);
        dots.setForeground(butf);
        fix(dots,grd,gbc,4,2,1,1,20,10);
	
	// Blank button 5
	
	lines = new JButton("Lines");
        lines.setBackground(butn);
        lines.setForeground(butf);
        fix(lines,grd,gbc,4,3,1,1,20,10);
	
	// Blank button 6
	
	prin = new JButton("Print");
        prin.setBackground(butn);
        prin.setForeground(butf);
        fix(prin,grd,gbc,4,4,1,1,20,10);
	
	// Blank button 7
	
	zoom = new JButton("Zoom");
        zoom.setBackground(butn);
        zoom.setForeground(butf);
	...
	xlabel=new JTextField("X axis");
        xlabel.setBackground(scrn);
        xlabel.setForeground(scrf);
        fix(xlabel,grd,gbc,0,8,1,1,5,10);

	// Y label textfield

	ylabel=new JTextField("Y axis");
        ylabel.setBackground(scrn);
        ylabel.setForeground(scrf);
        fix(ylabel,grd,gbc,1,8,1,1,5,10);

	// Plot label textfield

	plabel=new JTextField("Title");
        plabel.setBackground(scrn);
        plabel.setForeground(scrf);
...
*/
	home=here;
	ngrid=1000;
	monitor.println("Activated Graph Drawing panel");
	line=0;
	npnts=0;
	ldots=true;
	lzoom=false;
...
*/
	...
	if(e.getSource() instanceof JTextField)
	    ...
	    {
		if((fname=selectFileNameEnds(job,"XY"))!=null)
		    {
			keyplot=0;
			lzoom=false;
			lscale=false;
