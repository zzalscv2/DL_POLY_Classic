...
public class RunFAF extends Super implements ActionListener
{
	/*
*********************************************************************
...
*/
    ...
    static double[][] faf,xyz,vel,frc;
    static double[][][] faf0;
    static int npnts,nconf,nfaf,isampl,iofaf;
...
*/
	home=here;
	monitor.println("Activated FAF panel");
	job=new RunFAF();
	job.pack();
	job.show();
	npnts=0;
...
*/
	...
	if (arg.equals("Run"))
		...
		monitor.println("Started FAF calculation .....");
		npnts=calcFAF();
		if(npnts>0)
		    {
			fafXY(npnts,atname);
...
*/      
	...
	if(nfaf%iofaf != 0)
		...
		monitor.println("Warning - faf array dimension reset to "+BML.fmt(nfaf,8));
	    }
      	nofaf=nfaf/iofaf;
	msm=new int[nfaf];
	imd=new int[nfaf];
	...
	monitor.println("Length of correlation arrays  : "+BML.fmt(nfaf,8));
	monitor.println("Number of configurations      : "+BML.fmt(nconf,8));
	monitor.println("Sampling interval             : "+BML.fmt(isampl,8));
	monitor.println("Interval between origins      : "+BML.fmt(iofaf,8));
